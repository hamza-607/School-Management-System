...
<div class="card mb-4">
    <div class="card-body">
        <form action="{{ route('penalties.store', $theStudent->id) }}" method="POST">
            @csrf

            <div class="col-md-6">
                <label class="form-label">نوع العقوبة<span class="text-danger">*</span></label>
                <input class="form-control @error('penalty_type') is-invalid @enderror" type="text" name="penalty_type" value="{{ old('penalty_type') }}" required>
                @error('penalty_type')
