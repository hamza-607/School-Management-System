    ...
    <div class="card shadow-sm border-0 header-card">
        <div class="card-body">
            <div class="d-flex align-items-start gap-3 mb-3">
                <div>
                    <h5 class="mb-1" style="font-size:xx-large;">{{ $theSection->name }}</h5>

                    <div class="patient-details">
                        <div class="mb-1 text-muted" style="font-size:large;">
                            الصف: {{ $theSection->grade->name }}
                        </div>
                        <div class="mb-1 text-muted" style="font-size:large;">
                            السعة: {{ $theSection->students->count() }} / {{ $theSection->capacity }} طالب
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4 border-top border-bottom pt-3 pb-3">
                <button type="button" class="btn btn-label-danger px-4" onclick="confirmDelete({{ $theSection->id }})">
                    حذف المادة
                </button>

                <form id="delete-form-{{ $theSection->id }}" action="{{ route('sections.destroy', $theSection->id) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>

                <a href="{{ route('sections.edit', $theSection->id) }}"
            ...
            <div class="d-flex flex-nowrap overflow-x-auto gap-3" style="overflow-x: scroll; scrollbar-width: none;">
                ...
                <div class="parent-card border rounded-3 p-3 bg-light-subtle" style="min-width: 300px; flex: 0 0 auto;">
                    ...
                    <div class="mt-2 pt-2 border-top">
                        <div>
                            <a href="https://wa.me/{{ preg_replace('/\D/', '', $student->phone ) }}" target="_blank" class="text-decoration-underline me-3 small mb-1">
                                <i class="bi bi-whatsapp me-2 text-success"></i>
                                {{ $student->phoneNumber }} </a>
                        </div>
                    </div>
                </div>
                @empty
                <p class="text-muted">لا يوجد طلاب لهذه الشعبة.</p>
                @endforelse
            </div>
        </div>
    </div>

    <x-files-table :model="$theSection" mainTitle="ملفات ومرفقات الشعبة" secTitle="لا توجد ملفات مرفوعة ."></x-files-table>
