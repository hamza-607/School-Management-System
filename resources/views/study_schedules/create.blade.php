...
<script>
    $(document).ready(function() {
        ...
        $('#subject_select').on('change', function() {
            ...
            // 3. (مهم جداً) دائماً أضف خيار "إضافة مدرس" في نهاية القائمة
            $('#staff_select').append('<option value="add_new_teacher" class="text-primary fw-bold">✚ إضافة مدرس جديد</option>');

            // 4. إذا كنت قد أضفت مدرساً "جديداً" للتو (NEW)، أعد إضافته لكي لا يختفي عند تغيير المادة
    ...
    $('#confirmTeacher').click(function() {
        // جلب القيم من حقول المودال (عبر الـ ID)
        ...
        // نقل القيم إلى الحقول المخفية أسفل الفورم
        ...
        $('#teacher_e_name_input').val('');
        $('#teacher_phone_input').val('');
        $('#teacher_email_input').val('');
        $('#flatpickr-date').val('');
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // تفعيل Cleave على كل العناصر التي تحمل كلاس time-mask
...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">البرامج الدراسية / القائمة / برنامج الصف {{ $theSection->grade->name }} - الشعبة {{ $theSection->name }} / </span> إضافة حصة درسية جديدة
</h4>

<div class="card mb-4">
    <div class="card-body">
        <form action="{{ route('studySchedules.store', $theSection->id) }}" method="POST" id="studySessionForm" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="end_time" class="form-control time-mask" placeholder="23:59:59" required>
                    @error('staff')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
